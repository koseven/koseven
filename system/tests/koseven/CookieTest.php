 ...
 */
class Koseven_CookieTest extends Unittest_TestCase
{
	...
	{
		parent::setUp();
		Koseven_CookieTest_TestableCookie::$_mock_cookies_set = [];
	...
	public function test_set_creates_cookie_with_configured_cookie_options()
	{
		...
		Koseven_CookieTest_TestableCookie::set('cookie', 'value');
	...
	public function test_set_calculates_expiry_from_lifetime($expiration, $expect_expiry)
	{
		...
		Koseven_CookieTest_TestableCookie::set('foo', 'bar', $expiration);
	...
	{
		Koseven_CookieTest_TestableCookie::set('cookie', 'value');
		$cookie = Koseven_CookieTest_TestableCookie::$_mock_cookies_set[0];
	...
	/**
	 * Verifies that unsigned cookies are not available to the koseven application, but are not affected for other
	 * consumers.
	 *
	...
	public function test_get_returns_default_without_deleting_if_cookie_unsigned($unsigned_value)
	{
		...
		$this->assertEquals('default', Koseven_CookieTest_TestableCookie::get('cookie', 'default'));
		$this->assertEquals($unsigned_value, $_COOKIE['cookie'], '$_COOKIE not affected');
		$this->assertEmpty(Koseven_CookieTest_TestableCookie::$_mock_cookies_set, 'No cookies set or changed');
	}
	...
	{
		$_COOKIE['cookie'] = Cookie::salt('cookie', 'value').'~tampered';
		$this->assertTrue(Koseven_CookieTest_TestableCookie::delete('cookie'));
		$this->assertDeletedCookie('cookie');
	}

	/**
	 * @covers Cookie::delete
	 * @link    http://dev.kosevenframework.org/issues/3501
	 * @link    http://dev.kosevenframework.org/issues/3020
	 */
	...
	// @codingStandardsIgnoreEnd
	{
		$this->assertCount(1, Koseven_CookieTest_TestableCookie::$_mock_cookies_set);
		$relevant_values = array_intersect_key(Koseven_CookieTest_TestableCookie::$_mock_cookies_set[0], $expected);
...
/**
 * Class Koseven_CookieTest_TestableCookie wraps the cookie class to mock out the actual setcookie and time calls for
 * unit testing.
 */
class Koseven_CookieTest_TestableCookie extends Cookie {
	...
	{
		return Koseven_CookieTest::UNIX_TIMESTAMP;
	}

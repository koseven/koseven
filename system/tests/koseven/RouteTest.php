...
include Koseven::find_file('tests', 'test_data/callback_routes');

class Koseven_RouteTest extends Unittest_TestCase
{
	/**
	 * Remove all caches
	 */
	...
	// @codingStandardsIgnoreEnd
	{
		...
		Koseven::$config->load('url')->set('trusted_hosts', ['kosevenframework\.org']);
	 ...
	 * it should throw a Koseven_Exception
	 *
	...
	/**
	 * Check appending cached routes. See http://dev.kosevenframework.org/issues/4347
	 *
	 * @test
	 * @covers Route::cache
	 */
	...
	public function test_uri_throws_exception_if_required_params_are_missing($uri, $regex, $uri_array)
	{
        ...
        $this->expectException('Koseven_Exception');
		$route->uri($uri_array);
	}
	...
	public function provider_composing_url_from_route()
	{
		return [
			...
			['http://kosevenframework.org/news', ['controller' => 'news'], 'http']
		];
	}
	...
	public function test_composing_url_from_route($expected, $params = NULL, $protocol = NULL)
	{
		...
		$this->setEnvironment([
			'_SERVER' => ['HTTP_HOST' => 'kosevenframework.org'],
			'Koseven::$base_url' => '/',
			'Koseven::$index_file' => '',
		]);
	...
	public function test_is_external_route_from_host()
	{
		...
		Route::set('external', 'local/test/route')
			->defaults([
				'controller' => 'foo',
				'action'     => 'bar',
				'host'       => 'http://kosevenframework.org'
				]
			);
		...
		return [
			[
				...
				[
					'controller'  => 'foo',
					'action'      => 'bar',
					'host'        => 'kosevenframework.org'
				],
				'http://kosevenframework.org/foo/bar'
			],
			[
				'<controller>/<action>',
				[
	...
	/**
	 * http://dev.kosevenframework.org/issues/4079
	 *
	 * @test
	 * @covers Route::get
	 * @ticket 4079

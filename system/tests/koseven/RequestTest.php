 ...
 */
class Koseven_RequestTest extends Unittest_TestCase
{
	...
	public function test_param()
	{
		...
		$uri = 'koseven_requesttest_dummy/foobar/some_id';
		$request = Request::factory($uri, NULL, TRUE, [$route]);

		// We need to execute the request before it has matched a route
		$response = $request->execute();
		$controller = new Controller_Koseven_RequestTest_Dummy($request, $response);
		...
		$route->defaults(['controller' => 'koseven_requesttest_dummy', 'action' => 'foobar']);
		$request = Request::factory('koseven_requesttest_dummy', NULL, TRUE, [$route]);
		...
		$this->assertSame(200, $response->status());
		$this->assertSame($controller->get_expected_response(), $response->body());
		$this->assertSame('koseven_requesttest_dummy', $request->param('uri'));
	}
	...
	public function provider_url()
	{
		return [
			[
				'foo/bar',
				'http',
				'http://localhost/koseven/foo/bar'
			],
			[
				'foo',
				'http',
				'http://localhost/koseven/foo'
			],
			[
				'http://www.google.com',
				'http',
	...
	{
		$x_powered_by = 'Koseven Unit Test';
	...
	public function provider_client()
	{
		...
		return [
			[
				new Request('http://kosevenframework.org'),
				$internal_client,
				$internal_client
			],
			[
	...
	public function test_content_length_after_render()
	{
		$request = Request::factory('https://example.org/post')
			->client(new Koseven_RequestTest_Header_Spying_Request_Client_External)
			->method(Request::POST)
			->post(['aaa' => 'bbb']);
...
} // End Koseven_RequestTest

/**
 * A dummy Request_Client_External implementation, that spies on the headers
 * of the request
 */
class Koseven_RequestTest_Header_Spying_Request_Client_External extends Request_Client_External
{
	private $headers;
...
class Controller_Koseven_RequestTest_Dummy extends Controller
{

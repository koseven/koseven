<?php
/**
 * The Koseven_HTTP_Header class provides an Object-Orientated interface
 ...
 */
class Koseven_HTTP_Header extends ArrayObject {
	...
	/**
	 * Constructor method for [Koseven_HTTP_Header]. Uses the standard constructor
	 * of the parent `ArrayObject` class.
	 *
	 *     $header_object = new HTTP_Header(array('x-powered-by' => 'Koseven 3.1.x', 'expires' => '...'));
	 *
	...
	public function accepts_at_quality($type, $explicit = FALSE)
	{
		...
		{
			$mime = Koseven::$config->load('mimes.'.$type);
	...
	public function send_headers(HTTP_Response $response = NULL, $replace = FALSE, $callback = NULL)
	{
		...
		{
			$processed_headers[] = 'Content-Type: '.Koseven::$content_type.'; charset='.Koseven::$charset;
		}

		if (Koseven::$expose AND ! isset($headers['x-powered-by']))
		{
			$processed_headers[] = 'X-Powered-By: '.Koseven::version();
		}

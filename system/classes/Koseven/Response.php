 ...
 * @since      3.1.0
 */
class Koseven_Response implements HTTP_Response {
	...
	public function status($status = NULL)
	{
		...
		{
			throw new Koseven_Exception(__METHOD__.' unknown status value : :value', [':value' => $status]);
		}
	}
	 ...
	 *
	 *     $request->send_file('media/packages/koseven.zip');
	 *
	...
	public function send_file($filename, $download = NULL, array $options = NULL)
	{
		...
		if ($filename === TRUE)
		{
			if (empty($download))
			{
				throw new Koseven_Exception('Download name must be provided for streaming files');
			}
		...
		else if (is_resource($filename) && get_resource_type($filename) === 'stream')
		{
			...
			{
				throw new Koseven_Exception('Resource must be a file handle');
			}
		...
		if ( ! is_resource($file))
		{
			throw new Koseven_Exception('Could not read file to send: :file', [
				':file' => $download,
			]);
		}
		...
		if ( ! empty($options['delete']))
		{
			...
			catch (Exception $e)
			{
				...
				if (is_object(Koseven::$log))
				{
					...
					Koseven::$log->add(Log::ERROR, $error);
	...
	public function render()
	{
		if ( ! $this->_header->offsetExists('content-type'))
		{
			...
			$this->_header['content-type'] = Koseven::$content_type.'; charset='.Koseven::$charset;
		}

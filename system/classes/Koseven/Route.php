...
class Koseven_Route {
	...
	public static function get($name)
	{
		...
		{
			throw new Koseven_Exception('The requested route does not exist: :route',
				[':route' => $name]);
		}
	...
	public static function cache($save = FALSE, $append = FALSE)
	{
		if ($save === TRUE)
		{
			try
			{
				...
				Koseven::cache('Route::cache()', Route::$_routes);
			}
			catch (Exception $e)
			{
				// We most likely have a lambda in a route, which cannot be cached
				throw new Koseven_Exception('One or more routes could not be cached (:message)', [
		...
		{
			if ($routes = Koseven::cache('Route::cache()'))
			{
				if ($append)
				{
	...
	public function filter($callback)
	{
		...
		{
			throw new Koseven_Exception('Invalid Route::callback specified');
		}

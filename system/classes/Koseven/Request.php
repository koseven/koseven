 ...
 */
class Koseven_Request implements HTTP_Request {
	...
	public static function detect_uri()
	{
		...
		else
		{
			...
			else
			{
				...
				throw new Koseven_Exception('Unable to detect the URI using PATH_INFO, REQUEST_URI, PHP_SELF or REDIRECT_URL');
			}

			// Get the path from the base URL, including the index file
			$base_url = parse_url(Koseven::$base_url, PHP_URL_PATH);
			...
			if (Koseven::$index_file AND strpos($uri, Koseven::$index_file) === 0)
			{
				// Remove the index file from the URI
				$uri = (string) substr($uri, strlen(Koseven::$index_file));
			}
		}
	...
	/**
	 * @var  Koseven_HTTP_Header  headers to sent as part of the request
	 */
	protected $_header;
	...
	public function execute()
	{
		...
		{
			throw new Request_Exception('Unable to execute :uri without a Koseven_Request_Client', [
				':uri' => $this->_uri,
			]);
		}
	...
	/**
	 * Returns whether this request is the initial request Koseven received.
	 ...
	 *  - Body
	 *
	 *  If there are variables set to the `Koseven_Request::$_post`
	...
	public function render()
	{
		...
		else
		{
			...
			$this->body($body)
				->headers('content-type', 'application/x-www-form-urlencoded; charset='.Koseven::$charset);
		}

		// Set the content length
		$this->headers('content-length', (string) $this->content_length());

		// If Koseven expose, set the user-agent
		if (Koseven::$expose)
		{
			$this->headers('user-agent', Koseven::version());
		}

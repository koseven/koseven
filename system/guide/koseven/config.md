...
[Koseven_Config_Reader] / [Koseven_Config_Writer] interfaces:

	class Koseven_Config_Database_Reader implements Koseven_Config_Reader
	class Koseven_Config_Database_Writer extends Koseven_Config_Database_Reader implements Koseven_Config_Writer
...
To load a config group simply call `Koseven::$config->load()` with the name of the group you wish to load:

	$config = Koseven::$config->load('my_group');
...
from the config group to the value you want:
	...
	$hostname = Koseven::$config->load('database.default.connection.hostname');


Which is equivalent to:

	$config = Koseven::$config->load('database')->get('default');
...
syntax to get/set config vars:
	...
	$config = Koseven::$config->load('database');
...
By default when you load a source it is pushed to the top of a stack:
	...
	// Stack: Config_File
	Koseven::$config->attach(new Config_Database);
...
For example, using the setup outlined above:
	...
	// Configuration returned by Koseven::$config->load('email')
		email:
			sender:
				email: my.supercool.address@gmail.com
				name:  Koseven Bot
...
as the second parameter to `attach()`:
	...
	Koseven::$config->attach(new Config_File);
	// Stack: Config_File
	Koseven::$config->attach(new Config_Database, FALSE);
...
In some situations you'll need to use different config values depending on which state `Koseven::$environment`
...
"testing":
	...
	if (Koseven::$environment === Koseven::TESTING)
	{
		Koseven::$config->attach(new Config_File('config/testing'));
	}

During normal development the config source stack looks like `Config_Database, Config_File('config')`.  However,
when `Koseven::$environment === Koseven::TESTING` the stack looks like `Config_File('config/testing'), Config_Database, Config_File('config')`